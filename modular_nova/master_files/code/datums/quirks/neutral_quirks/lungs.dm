...
/datum/quirk/equipping/lungs/add(client/client_source)
    ...
    if(!isnull(lungs_holding))
        lungs_holding.organ_flags |= ORGAN_FROZEN // parar a decomposição dos pulmões antigos
    lungs_added = new lungs_typepath
    lungs_added.Insert(carbon_holder, special = TRUE)
    if(!isnull(lungs_holding))
        lungs_holding.moveToNullspace() // guardá-los para mais tarde
...
/obj/item/clothing/accessory/breathing
    name = "Placa de identificação do que respira"
    desc = "Placa de identificação que lista o que você respira."
    icon_state = "allergy"
    above_suit = FALSE
...
/obj/item/clothing/accessory/breathing/examine(mob/user)
    . = ..()
    . += "A Placa de identificação diz: Eu respiro [breath_type]."
...
/obj/item/clothing/accessory/breathing/proc/on_examine(datum/source, mob/user, list/examine_list)
    SIGNAL_HANDLER
    examine_list += "A Placa de identificação diz: Eu respiro [breath_type]."

/datum/quirk/equipping/lungs/nitrogen
    name = "Respirador de Nitrogênio"
    desc = "Você respira nitrogênio, mesmo que normalmente não respire. Oxigênio é venenoso."
    icon = FA_ICON_LUNGS_VIRUS
    medical_record_text = "O paciente só pode respirar nitrogênio."
    gain_text = "<span class='danger'>Você de repente tem dificuldade em respirar qualquer coisa além de nitrogênio.</span>"
    lose_text = "<span class='notice'>Você de repente sente que não está mais preso ao nitrogênio.</span>"
    value = 0
    forced_items = list(
